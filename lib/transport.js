...
function fromCfg(connectionStringOrConfig) {
    ...
    if (_.isString(connectionStringOrConfig)) {
    ...
    else if (connectionStringOrConfig.type) {
        connectionCfg = _.omit(connectionStringOrConfig, 'type');
        switch(connectionStringOrConfig.type) {
            case "mysql":
                transport = MySQLTransport(connectionCfg);
                break;
            default:
                throw new Error(
        ...
        throw new Error(
            ...
            "since it has no `type` property. See https://github.com/talmago/node-mysql-migrate " +
            "for more details."
        );
    }
