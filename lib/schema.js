...
var SchemaManager = (function () {
    ...
    function SchemaManager(schema, transport) {
        ...
        _private._createObjects = function () {
            _private._logger.info("Creating Schema `%s` if not exists.", _private._schema);
            return _private._transport.execute(_private._escape("CREATE SCHEMA IF NOT EXISTS ??"))
                .then(function() {
                    _private._logger.info("Creating `%s` object in `%s` schema.", _private._revisionTbl, _private._schema);
                    return _private._transport.execute(_private._escape(
        ...
        _private._deleteObjects = function () {
            _private._logger.info("Deleting objects in `%s`.`%s`", _private._schema, _private._revisionTbl);
            return _private._transport.execute(_private._escape("DELETE FROM ??.??"));
        };

        // drop objects from DB
        _private._dropObjects = function () {
            _private._logger.info("Dropping objects in `%s`", _private._schema);
            return _private._transport.execute(_private._escape("DROP TABLE IF EXISTS ??.??"));
        };
        ...
        _private._getObjects = function () {
            ...
            return _private._transport.execute(_private._escape("SELECT * FROM ??.??"))
                .map(function(row) {
        ...
        _private._addObject = function (obj) {
            ...
            return _private._transport.execute(
