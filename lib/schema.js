...
var SchemaManager = (function () {
    ...
    function SchemaManager(schema, config) {
        ...
        _private._createObjects = function () {
            _private._logger.info("Creating Schema `%s` if not exists.", self._schema);
            return _private._client.execute(_private._escape("CREATE SCHEMA IF NOT EXISTS ??"))
                .then(function() {
        ...
        _private._migration = function (migration) {
            ...
            if (migration.type == "SQL") {
                ...
                promise = _private._client._getConnection()
            ...
            .then(function(objects) {
                ...
                return Util.discovery(directory)
                    ...
                    .then(function(scripts) {
                        var script;
                        _.each(fixUs, function(fixIt) {
                            script = _.find(scripts, function(s) {
                                return s.script == fixIt.script;
                            });
                            if (script) {
                                _private._logger.info("Attempting to repair %s/%s", fixIt);
                            } else {
                                return Promise.resolve();
                            }
                        });
